<?php
    ...
    if (isset($_POST['crop_id'], $_POST['season_id'], $_POST['quantity'], $_POST['user_id'])) {
        ...
        if ($item->createDistribution($crop_id, $season_id, $quantity, $user_id)) {
            // Alert created successfully
            http_response_code(201);
            echo json_encode(array("message" => "User Crop Distribution Created Successfully."));
        } else {
            // Failed to create the alert
            http_response_code(500);
            echo json_encode(array("message" => "Failed to create Crop Distribution Item."));
        }
    } else {
        // Required data not provided in the request
        http_response_code(400);
        echo json_encode(array("message" => "Missing required data for creating Crop Distribution."));
    }
?>